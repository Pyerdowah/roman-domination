...
Algorytm ten został zaprojektowany i zaimplementowany samodzielnie.
...
Poniżej przedstawiona zostanie analiza poprawności algorytmu BruteForce. Algorytm sprawdza wszystkie możliwe funkcje kandydujące, gwarantując kompletność przeszukania oraz poprawność weryfikacji kombinacji względem definicji WCRDF.\\
...
Algorytm ten został zaimplementowany samodzielnie w oparciu o znaną konstrukcję algorytmu mrówkowego.
...
Poniżej przedstawiona zostanie analiza poprawności algorytmu Approx. Algorytm opiera się na wyznaczeniu spójnego zbioru dominującego (CDS) przy pomocy programowania całkowitoliczbowego, a następnie przypisaniu wszystkim wierzchołkom należącym do tego zbioru wartości \( f(v) = 2 \), a pozostałym \( f(v) = 0 \). Wystarczy zatem wykazać, że model wyznacza spójny zbiór dominujący, a takie przypisanie będzie spełniało definicję WCRDF, ale niekoniecznie optymalnie.\\
...
\begin{algorithm}[H]
    ...
    Algorytm ten został zaimplementowany samodzielnie.
