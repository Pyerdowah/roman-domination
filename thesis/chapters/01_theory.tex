...
\section{Problemy i hipotezy badawcze}

Z racji tego, że znajdowanie WCRDF jest problemem NP-zupełnym, w wersja minimalizacyjna tego problemu, czyli znalezienie $\gamma_{R}^{\text{wc}}(G)$ jest NP-trudne, nie są znane zatem dokładne algorytmy rozwiązujące i weryfikujące problem w czasie wielomianowym. W związku z tym nasuwa się pierwsze pytanie badawcze:

\textbf{Jakie algorytmy są w stanie znaleźć WCRDF? Które z nich są w stanie znaleźć dodatkowo najmniejszą $\gamma_{R}^{\text{wc}}(G)$?}

Już podczas dowodu złożoności tego problemu należało rozpatrzeć przypadki dla różnych klas grafów. Stąd kolejne pytanie:

\textbf{Czy czas i jakość działania algorytmów będzie uzależnione od klasy grafów?}
...
Na podstawie przeglądu literatury można zauważyć, że WCRDP nie jest powszechnie znany i poza prezentowanymi powyżej publikacjami nie znaleziono więcej godnych uwagi pozycji dotyczących konkretnie tej wersji problemu. 
