...
\end{tikzpicture}
\caption{Rysunek poglądowy konstrukcji grafu dwudzielnego dla zadanych $X$ i $C$. Na czerwono zaznaczono wierzchołki należące do zbioru dominującego.}
\end{center}
\end{figure}

\section{Problemy i hipotezy badawcze}

Z racji tego, że znajdowanie WCRDF jest problemem NP-zupełnym, w wersja minimalizacyjna tego problemu, czyli znalezienie $\gamma_{R}^{\text{wc}}(G)$ jest NP-trudne, nie są znane zatem dokładne algorytmy rozwiązujące ten problem w czasie wielomianowym. Weryfikacja rozwiązania wersji optymalizacyjnej jest również trudna obliczeniowo. W związku z tym nasuwa się pierwsze pytanie badawcze:

\textbf{Jakie algorytmy są w stanie znaleźć WCRDF? Które z nich są w stanie znaleźć dodatkowo $\gamma_{R}^{\text{wc}}(G)$?}
